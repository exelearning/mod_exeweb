 ...
 *
 * eXeLearning only accepts return urls living under /mod/exeweb, so this script is the entry point
 * that lets the "Edit on eXeLearning and return to course" button reach the course page. All the
 * routing rules live in exeonline_redirector, which is the side that builds this url in the first
...
$returnurl = optional_param(exeonline_redirector::RETURNTO_PARAM, '', PARAM_LOCALURL);

$cm = get_coursemodule_from_id('exeweb', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', ['id' => $cm->course], '*', MUST_EXIST);

$PAGE->set_url(new moodle_url('/mod/exeweb/returnto.php', ['id' => $cm->id]));

require_login($course, false, $cm);

redirect(exeonline_redirector::resolve_returnto_url($returnurl, $course));
