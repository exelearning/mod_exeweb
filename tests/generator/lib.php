...
/**
 * Exeweb module data generator class.
 *
 * @package    mod_exeweb
 * @copyright 2013 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_exeweb_generator extends testing_module_generator {

    /**
     * Creates new exeweb module instance. By default it contains a short
     * text file.
     *
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        if (!isset($record->files)) {
            if (empty($USER->username) || $USER->username === 'guest') {
                throw new coding_exception('exeweb generator requires a current user');
            }
            $usercontext = context_user::instance($USER->id);
            $filename = $record->defaultfilename ?? 'exeweb' . ($this->instancecount + 1) . '.txt';
            ...
            if ($record->uploaded == 1) {
                ...
                // If defaultfilename is not set, create file from string "exeweb 1.txt".
                $fs->create_file_from_string($filerecord, 'Test exeweb ' . $filename . ' file');
            }
        }
