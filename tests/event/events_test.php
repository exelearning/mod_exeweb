...
namespace mod_exeweb\event;

/**
 * Exeweb events test cases.
 *
 * @package    mod_exeweb
 * @copyright  2014 Rajesh Taneja <rajesh@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class events_test extends \advanced_testcase {
    ...
    public function test_course_module_viewed() {
        ...
        $this->assertEquals(\context_module::instance($exeweb->cmid), $event->get_context());
        $this->assertEquals($exeweb->id, $event->objectid);
        $expected = [$course->id, 'exeweb', 'view', 'view.php?id=' . $exeweb->cmid, $exeweb->id, $exeweb->cmid];
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
}
