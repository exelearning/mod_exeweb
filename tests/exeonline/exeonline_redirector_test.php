...
namespace mod_exeweb\exeonline;
...
/**
 * Tests for mod_exeweb\exeonline\exeonline_redirector.
 *
 * @covers \mod_exeweb\exeonline\exeonline_redirector
 */
class exeonline_redirector_test extends \advanced_testcase {
    ...
    protected function setUp(): void {
        ...
        set_config('exeonlinebaseuri', 'https://exelearning.example.com', 'exeweb');
        set_config('hmackey1', 'testkey', 'exeweb');
        set_config('tokenexpiration', 300, 'exeweb');
    }
    ...
    public function test_module_return_url_is_kept_and_gets_the_cmid(): void {
        $url = exeonline_redirector::get_redirection_url(
            self::CMID,
            new moodle_url('/mod/exeweb/view.php', ['id' => 1, 'forceview' => 1])
        );

        $returnurl = $this->get_payload_returnurl($url);
        $this->assertStringContainsString('/mod/exeweb/view.php', $returnurl);
    ...
    public function test_course_return_url_is_routed_through_the_module(): void {
        ...
        $url = exeonline_redirector::get_redirection_url(self::CMID, $courseurl);
        ...
        $returnurl = $this->get_payload_returnurl($url);
        $this->assertStringStartsWith($CFG->wwwroot . '/mod/exeweb/returnto.php', $returnurl);
        $this->assertStringContainsString('id=' . self::CMID, $returnurl);
    ...
    public function test_default_return_url_resolves_to_the_activity_course(): void {
        ...
        $wrapped = new moodle_url($this->get_payload_returnurl(
            exeonline_redirector::get_redirection_url(self::CMID)
        ));

        $this->assertSame($CFG->wwwroot . '/mod/exeweb/returnto.php', $wrapped->out_omit_querystring());
        $this->assertSame(self::CMID, (int)$wrapped->param('id'));
        $this->assertNull($wrapped->param(exeonline_redirector::RETURNTO_PARAM));
        $this->assertSame(
            course_get_url($course)->out(false),
            exeonline_redirector::resolve_returnto_url('', $course)->out(false)
        );
    }
    ...
    public function test_external_return_url_is_dropped_and_resolves_to_the_course(): void {
        ...
        $wrapped = new moodle_url($this->get_payload_returnurl(
            exeonline_redirector::get_redirection_url(self::CMID, new moodle_url('https://not-this-moodle.invalid/foo'))
        ));
    ...
    public function test_resolve_returnto_url_restores_the_carried_destination(): void {
        ...
        $course = $this->getDataGenerator()->create_course();
        $resolved = exeonline_redirector::resolve_returnto_url('/course/view.php?id=' . $course->id, $course);
    ...
    public function test_resolve_returnto_url_falls_back_to_the_course(): void {
        ...
        $course = $this->getDataGenerator()->create_course();
        $resolved = exeonline_redirector::resolve_returnto_url('', $course);
